function place_speedy(){
	...
	all.pieces.attachMovie('speedy','enemy'+enemy_lev,pieces_lev+2000);
	...
	enemy_trg.id = 'enemy';
	enemy_trg.type = 'speedy';
	enemy_trg.state = 'levels';
	enemy_trg.health = boss2_health;
	enemy_trg.damage = Number(vars.speedy_damage);
...
function speedy_move(trg){
	...
	if(trg.vel == 0 && trg.centre_y == 35 && !trg.hit && !trg.dead && !trg.attack_hold){
		...
		if(!trg.no_attacking && trg.state == 'levels' && trg.old_level_fire != trg.my_level){
			...
			//fix(40,['speedy_fire_bullet',[trg]]);
			speedy_fire_bullet(trg);
		}
...
function speedy_fire_bullet(trg){
	play_fx('tt_speedy_archer',0,100)
	...
	trg.attack_wait.onEnterFrame = function(){
		...
		if(trg.attack_wait.wait == 20){
			...
			remove_onTickEvent_speedy();
		}
	}

	PIXI.ticker.shared.add(add_onTickEvent_speedy, trg.attack_wait.onEnterFrame);
	function add_onTickEvent_speedy(detla) {
		if (trg.attack_wait.onEnterFrame != undefined)
		trg.attack_wait.onEnterFrame();
	}
	function remove_onTickEvent_speedy() {
		PIXI.ticker.shared.remove(add_onTickEvent_speedy, trg.attack_wait.onEnterFrame);
	}
	...
	all.pieces.attachMovie('speedy_arrow','bullet'+bullet_lev,pieces_lev+5000);
	...
	all.BulletFunc['bullet'+bullet_lev] = fire_bullet_ef.bind(trg.damage, bullet_trg, this);
