function place_bumble_bee(){
	...
	all.pieces.attachMovie('bumble_bee','enemy'+enemy_lev,pieces_lev+2000);
	...
	enemy_trg.id = 'enemy';
	enemy_trg.type = 'bumble_bee';
	enemy_trg.state = 'levels';
	enemy_trg.health = boss2_health;
	enemy_trg.damage = Number(vars.bumble_bee_damage);
...
function bumble_bee_move(trg){
	...
	if(trg.vel == 0 && trg.centre_y == 35 && !trg.hit && !trg.dead && !trg.attack_hold){
		...
		if(!trg.no_attacking && trg.state == 'levels' && trg.old_level_fire != trg.my_level){
			...
			fix(40,['bumble_bee_fire_bullet',[trg]]);
		}
...
function bumble_bee_fire_bullet(trg){
	play_fx('tt_bumblebee_b_stingers',0,100);
	...
	trg.attack_wait.onEnterFrame = function(){
		...
		if(trg.attack_wait.wait == 20){
			...
			remove_onTickEvent_bumble_bee();
		}
	}

	PIXI.ticker.shared.add(add_onTickEvent_bumble_bee, trg.attack_wait.onEnterFrame);
	function add_onTickEvent_bumble_bee(detla) {
		if (trg.attack_wait.onEnterFrame != undefined)
		trg.attack_wait.onEnterFrame();
	}
	function remove_onTickEvent_bumble_bee() {
		PIXI.ticker.shared.remove(add_onTickEvent_bumble_bee, trg.attack_wait.onEnterFrame);
	}
	...
	all.pieces.attachMovie('bee_bolt','bullet'+bullet_lev,pieces_lev+5000);
