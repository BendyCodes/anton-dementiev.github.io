...
function bee_fire_bullet(trg){
	play_fx('tt_turrets_lasers',0,100);
	pieces_lev++;
	bullet_lev++;
	all.pieces.attachMovie = function(name, piece) {
	    all_pieces_attachMovie(name, piece);
	}
	all.pieces.attachMovie('bee_fire','bullet'+bullet_lev,pieces_lev+5000);
	...
	bullet_trg.onEnterFrame = function(){
		bullet_ai(bullet_trg,trg.damage,40,8);
	}

	all.BulletFunc['bullet_'+trg.bullet_id] = bullet_trg.onEnterFrame;
	PIXI.ticker.shared.add(all.BulletFunc['bullet_'+trg.bullet_id], 'bullet_'+trg.bullet_id);
}

function bee_movement_ai(trg){
	// centres
	trg.centre_x = trg._x;
