...
function patrol_fire_bullet(trg){
	play_fx('tt_beetrooper_lasers',0,100);
	pieces_lev++;
	bullet_lev++;
	all.pieces.attachMovie = function(name, piece) {
	    all_pieces_attachMovie(name, piece);
	}
	all.pieces.attachMovie('bullet','bullet'+bullet_lev,pieces_lev+5000);
	...
	bullet_trg.onEnterFrame = function(){
		bullet_ai(bullet_trg,trg.damage,20,30);
	}
...
function patrol_movement_ai(trg){
	// centres
	trg.centre_x = trg._x;
