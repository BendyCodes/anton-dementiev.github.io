...
GodStep.Game = function(n, color, preloaderClass, setting_slot, player_slot, defaultSettings, type, w, h) {
    this.ANTIALIAS = true;
    GodStep.PAUSE = false;
    GodStep.BOT_OFFSET = 0;
    GodStep.ORIENTATION_PORTRAIT = true;
    ...
    this.renderer.backgroundColor  = GodStep.COLOR_STAGE = color;
...
pro.getFpsDelta = function (isUpdate) {
    var d = Games.FRAME_RATE;
    var newUpdateTime = new Date().getTime();
    if(this.lastUpdateTime < 0) this.lastUpdateTime = newUpdateTime;
    var delta = (this.lastUpdateTime - newUpdateTime)/1000;
    if(isUpdate) this.lastUpdateTime = newUpdateTime;
...
function h_resize() {
  //  trace('resize');
   ...
   // alert(game.div.style.top + " " + document.documentElement.clientWidth  + "\n" + document.documentElement.clientHeight + '\n' + window.innerHeight + "\n" + document.body.clientWidth + "\n" + document.body.clientHeight + "\n");
    /*
    ...
    if (document.body && (document.body.clientWidth || document.body.clientHeight)) {
        height = document.body.clientHeight;//Math.max(height = document.body.clientHeight, height);
    }
    //*/
   // game.resizeWindow(window.outerWidth, window.outerHeight);//height - GodStep.BOT_OFFSET);
  //  game.resizeWindow(12, 12);
    //game.resizeWindow(window.innerWidth, Math.max(window.outerHeight, window.innerHeight));
    game.resizeWindow(document.body.clientWidth, height - GodStep.BOT_OFFSET);
   // if(game.resizeFunction) {
       // game.resizeFunction(document.body.clientWidth - 1, height - 1);
   // }


}
pro.initResizeEvents = function(func){
    var supportsOrientationChange = false;
    var orientationEvent = supportsOrientationChange ? "orientationchange" : "resize";

    this.resizeFunction = func;
    window.addEventListener("resize", function() {
         setTimeout(h_resize, 250);
       //  h_resize();
    }, false);
  ...
  //  trace('S : ' + name);
    ...
    if (sound.AVAILABLE) {
        if (GodStep.Game.instance.AUDIOTAG) {
            ...
            if (isOk) {
                var s = createjs.Sound.play(name, createjs.Sound.PLAY_SUCCEEDED, 0, 0, repeatCount, null,1);
                s.superName = name;
...
GodStep.pause = function () {
    GodStep.PAUSE = true;
};
GodStep.unpause = function () {
    GodStep.PAUSE = false;
};
